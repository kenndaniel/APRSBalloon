...
unsigned long freqAPRSHigh;
unsigned long freqAPRSLow;
...
void setFrequencyAPRS()
{
    unsigned long Deviation = 4900; 
    unsigned long freqAPRS = (unsigned long) (GEOFENCE_APRS_frequency*(correction));
    freqAPRSHigh = freqAPRS + Deviation;
    freqAPRSLow = freqAPRS - Deviation;
      POUTPUT(F("APRS Frequency "));
      POUTPUTLN((freqAPRS));
...
void APRSon()
{
    si5351.output_enable(SI5351_CLK0, 1); // enable the output
}
...
{ 
if (MODE == LOW)
    si5351.set_freq((freqAPRSLow * 100), SI5351_CLK0);  // send high tone
else
    si5351.set_freq((freqAPRSHigh * 100), SI5351_CLK0); // send low tone
}
